...
class MySpider(CrawlSpider):
	...
	#Store data into lists
			...
			url = 'http://iva.se' + url
			item['url'] = url
			item['description'] = div.xpath('//div/div/div[1]/div[2]/figure/div/p[2]/text()').extract()[i]

	#Remove unwanted characters
			location = div.xpath('//div/div/div[2]/section/ul/li[1]/text()').extract()[i]
			item['location'] = location.strip()

	#The following code changes the format of the date
			origDate = div.xpath ('//div/div/div[2]/section/ul/li[2]/text()').extract()[i]
	...
	#Puts everything together and stores into item['date']
			...
			if i < len(response.xpath('/html/body/main/div/div/article')):
			 	i = i + 1
			 	yield request
